...
def listdir(folder, prepend_folder=False, extension=None, type=None):
  ...
  files = [k for k in os.listdir(folder) if (True if extension is None else k.endswith(extension)) and k != 'classes.txt']
  if type == 'folder':
...
def main(thresh, k, debug=False):
  # list imagenet100
  IMAGENET_PATH='/vision-nfs/torralba/datasets/vision/imagenet-pytorch'
  train_image_paths = []
  test_image_paths = []
  ...
  features_train_path = 'computed_features/features_train_th_{}{}.npy'.format(thresh, '_debug' if debug else '')
...
if __name__ == '__main__':
  ...
  for th in [35]:
    for k in [256]:
      acc_file = 'results/acc_th_{}_k_{}{}.txt'.format(th, k, '_debug' if debug else '')
      os.makedirs('results', exist_ok=True)
      if os.path.exists(acc_file):
        print("Already computed for th={}, k={}, with accuracy:".format(th, k))
        print(read_text_file(acc_file))
        continue
      print('\nCalculating for a threshold of {}, k={}'.format(th, k))
      data = main(th, k, debug=debug)
      accuracy = data[0]
      conf_mat = data[1]
      timer = data[2]
