    ...
    if utils.get_rank() == 0:
        ...
        if not args.dump_nn_images_path is None:
            for split in ['train', 'val']:
                dataset = ReturnIndexDataset(os.path.join(args.data_path, split), transform=None)
                filenames = [k[0] for k in dataset.samples]
        ...
        best_acc1 = 0
        for video_reader in args.nb_knn:
            top1, top5 = knn_classifier(train_features, train_labels, train_filenames,
                test_features, test_labels, test_filenames, video_reader, args.temperature, dump_images_path=args.dump_nn_images_path)
            print(f"{video_reader}-NN classifier result: Top1: {top1}, Top5: {top5}")
            if best_acc1 < top1:
                best_acc1 = top1
