...
torchrun --master_port $MASTER_PORT --nproc_per_node=$N_GPUS main_dino.py --arch vit_base \
...
if [ ! -f encoders/$EXPERIMENT_NAME/checkpoint/knn/best_acc.txt ]; then
    ...
    torchrun --master_port $MASTER_PORT --nproc_per_node=1 eval_knn.py --arch vit_base --pretrained_weights encoders/$EXPERIMENT_NAME/checkpoint.pth \
