...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, model_name, patch_size, pretraining_epochs=-1):
    if os.path.isfile(pretrained_weights):
        state_dict = torch.load(pretrained_weights, map_location="cpu")
        if pretraining_epochs != -1:
            # for sanity checks
            assert state_dict['epoch'] == pretraining_epochs, "Tried to load a checkpoint with desired number of pretraining epochs is {} but the checkpoint has {} epochs".format(pretraining_epochs, state_dict['epoch'])
            print("Will load the checkpoint with {} pretraining epochs".format(pretraining_epochs))
